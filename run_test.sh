...
run_program() {
  echo "Running alg.: $1"
  docker run --rm -v $(pwd)/1-data:/data:ro -v $(pwd)/2-results:/results:rw $1:latest execute-algorithm '{"executionType": "train", "dataInput": "/data/Wave_Height_test.csv", "dataOutput": "/results/$1_WaveHeight.ts", "modelInput": "/results/$1.pkl", "modelOutput": "/results/$1.pkl"}' &
  wait
  docker run --rm -v $(pwd)/1-data:/data:ro -v $(pwd)/2-results:/results:rw $1:latest execute-algorithm '{"executionType": "execute", "dataInput": "/data/Wave_Height_test.csv", "dataOutput": "/results/$1_WaveHeight.ts", "modelInput": "/results/$1.pkl", "modelOutput": "/results/$1.pkl"}' &
  wait
  ((running_processes--))
}
